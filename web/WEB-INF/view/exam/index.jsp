...
<body>
    ...
    <div class="tools">  
     <ul class="toolbar">
        <li>
         <form id="conditionForm" action="<%=request.getContextPath() %>/ExamManageServlet?command=listExamManageVerifyByPage" method="post">
        ...
        <ul class="toolbar1">
        <li class="click"><a href="<%=request.getContextPath() %>/ExamManageServlet?command=addExamManagePage" target="rightFrame"><span><img src="<%=request.getContextPath() %>/static/common/images/t01.png" /></span>添加考试安排</a></li>
        </ul>   
    </div>	
		...
		<c:forEach items="${pageModel.list }" var="MangeMessage" varStatus="v">
			<tr>
				...
				<td>
					...
					<c:if test="${MangeMessage.state == 1}">
					<a href="javascript:deleteMnage(${MangeMessage.state }, ${MangeMessage.id });" onclick="return confirm('考试安排已执行，是否确定删除？');">删除</a>
					</c:if>
					<c:if test="${MangeMessage.state == 2}">
