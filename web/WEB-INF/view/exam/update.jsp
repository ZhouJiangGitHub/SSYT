...
<body>
    ...
    <form action="<%=request.getContextPath() %>/ExamManageServlet?command=changManageMessege&Id=${examDetailEntity.id}" method="post">
    <ul class="forminfo">
    ...
    <li><label>考试教室：</label><input type="text" class="dfinput" name="classroom"
					id="classroom" onblur="validateclassroom();" value="${examDetailEntity.exam_classroom }"/><span
	...
	<li><label>操作者：</label><input type="text" class="dfinput" name="operate_user" id="operate_user"
					onblur="validateOperate_user();" value="${examDetailEntity.operate_user_id }" onfocus="clearOperate_userText();" /><span id="operate_userInfo"></span></li>
   <%--  <li><label>状态：</label><input type="text" class="dfinput" name="state"
    ...
    <li><label></label><button type="submit"  class="btn btn-primary">确认修改</button></li>
    </ul>
    </form> 
   </div>
</html>
