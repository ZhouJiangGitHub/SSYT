...
<li class="click"><a href="<%=request.getContextPath() %>/QuestionServlet?command=addTest" target="rightFrame">添加用户</a>
</li>
<form id="conditionForm" action="<%=request.getContextPath() %>/QuestionServlet?command=listPage" method="post">
    题目：<input type="text" class="form-control" name="question" value="${que.question }"
              style="width:100px ; display: inline">
    题型：<input type="text" class="form-control" name="questionType" value="${que.questionType }"
              style="width:100px ; display: inline">
    所属课程：<select name="sysCourse$Name">
    <option value=""
            <c:if test="${que.sysCourse$Name == ''}">selected="selected"</c:if>>-全部-
    </option>
    <option value="1"
            <c:if test="${que.sysCourse$Name == 1}">selected="selected"</c:if>>PHP
    </option>
    <option value="2"
            <c:if test="${que.sysCourse$Name == 2}">selected="selected"</c:if>>java
    </option>
    <option value="3"
            <c:if test="${que.sysCourse$Name == 3}">selected="selected"</c:if>>数字艺术
    </option>
    <option value="4"
            <c:if test="${que.sysCourse$Name == 4}">selected="selected"</c:if>>.net
    </option>
    <option value="5"
            <c:if test="${que.sysCourse$Name == 5}">selected="selected"</c:if>>android
    </option>
    <option value="6"
            <c:if test="${que.sysCourse$Name == 6}">selected="selected"</c:if>>ios
    </option>
</select>
    难度：<input type="text" class="form-control" name="difficulty" value="${que.difficulty }"
              style="width:100px ; display: inline">
