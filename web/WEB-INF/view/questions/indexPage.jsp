...
<head>
    ...
    <script type="text/javascript"
            src="<%=request.getContextPath() %>/static/common/plugin/layer-2.1/layer.js"></script>
    <title>无标题文档</title>
</head>
<body>
<div class="place">
...
<br/>
<div align="right">
    <li class="click"><a href="<%=request.getContextPath() %>/QuestionServlet?command=addTest"
                         target="rightFrame"><span><img
            src="<%=request.getContextPath() %>/static/common/images/t05.png"/>试题新增</span></a></li>
</div>
<form id="conditionForm" action="<%=request.getContextPath() %>/QuestionServlet?command=listParameterPage"
      method="post">
    题目：<input type="text" class="form-control" name="question" style="width:100px ; display: inline">
    题型：<select name="questionType">
    <option value="0">-全部-</option>
    <option value="1"
            <c:if test="${questionEn.questionType == 1}">selected="selected"</c:if>>单选题
    </option>
    <option value="2"
            <c:if test="${questionEn.questionType == 2}">selected="selected"</c:if>>多选题
    </option>
    <option value="3"
            <c:if test="${questionEn.questionType == 3}">selected="selected"</c:if>>判断题
    </option>
</select>
    所属课程：<select name="courseId">
    <option value="0">-全部-</option>
    <option value="1"
            <c:if test="${questionEn.courseId == 1}">selected="selected"</c:if>>PHP
    </option>
    <option value="2"
            <c:if test="${questionEn.courseId == 2}">selected="selected"</c:if>>java
    </option>
    <option value="3"
            <c:if test="${questionEn.courseId == 3}">selected="selected"</c:if>>数字艺术
    </option>
    <option value="4"
            <c:if test="${questionEn.courseId == 4}">selected="selected"</c:if>>.net
    </option>
    <option value="5"
            <c:if test="${questionEn.courseId ==5}">selected="selected"</c:if>>android
    </option>
    <option value="6"
            <c:if test="${questionEn.courseId == 6}">selected="selected"</c:if>>ios
    </option>
</select>
    难度：<select name="difficulty">
    <option value="0">-全部-</option>
    <option value="1"
            <c:if test="${questionEn.difficulty == 1}">selected="selected"</c:if>>低
    </option>
    <option value="2"
            <c:if test="${questionEn.difficulty == 2}">selected="selected"</c:if>>中
    </option>
    <option value="3"
            <c:if test="${questionEn.difficulty == 3}">selected="selected"</c:if>>较高
    </option>
    <option value="4"
            <c:if test="${questionEn.difficulty == 4}">selected="selected"</c:if>>高
    </option>
</select>
    状态：<select name="state">
    <option value="0">-全部-</option>
    <option value="1"
            <c:if test="${questionEn.state == 1}">selected="selected"</c:if>>有效
    </option>
    <option value="2"
            <c:if test="${questionEn.state == 2}">selected="selected"</c:if>>无效
    </option>
</select>
...
<table border="1px" class="table">
    ...
    <c:forEach items="${pageModel.list }" var="question" varStatus="v">
        <tr>
            ...
            </a><a href="<%=request.getContextPath()%>/QuestionServlet?command=details&id=${question.id}">详情</a> | <a
                    href="<%=request.getContextPath() %>/QuestionServlet?command=getMessage&id=${question.id}">修改</a>
            </td>
        </tr>
    </c:forEach>
    <tr>
