...
<dl class="leftmenu">
    <c:forEach items="${mainMenus }" var="mainMenu">
        <dd>
            <div class="title">
                <span><img
                        src="<%=request.getContextPath() %>/static/common/images/leftico01.png"/></span>${mainMenu.rightname }
            </div>
            <ul class="menuson">
                <c:forEach items="${childMenus }" var="childMenu">
                    <c:if test="${childMenu.parentId == mainMenu.rightId}">

                        <li><cite></cite><a href="${childMenu.righturl }"
                                            target="rightFrame">${childMenu.rightname }</a><i></i></li>
