...
<div align="left">
    <form action="<%=request.getContextPath() %>/RoleServlet?command=update&id=${sysRoleEntity.id}" method="post"
          onsubmit="return roleUpdate();">
        <li><label>角色名称：</label><input type="text" class="dfinput" name="name" id="name" onblur="validateName();"
                                       value="${sysRoleEntity.name }"/><span id="nameInfo"></span></li>
        <li><label>状　　态：</label><select name="state" id="state">
            <option value="1">启用</option>
            <option value="2">禁用</option>
        </select>
        </li>
        <li><label>备　　注：</label><input type="text" class="dfinput" name="memo" id="memo"
                                       value="${sysRoleEntity.memo }"/></li>
