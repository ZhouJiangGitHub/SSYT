...
<div align="right">
    <li class="click"><a href="<%=request.getContextPath() %>/RoleServlet?command=addTest"
                         target="rightFrame"><span><img
            src="<%=request.getContextPath() %>/static/common/images/t05.png"/>新增角色</span></a></li>
</div>
<form id="myForm" action="<%=request.getContextPath() %>/RoleServlet?command=list" method="post">
    状态：<select name="state" onchange="submit();">
    <option value="0"
            <c:if test="${sysRoleEntity.state == 0}">selected="selected"</c:if>>-全部-
    </option>
    <option value="1"
            <c:if test="${sysRoleEntity.state == 1}">selected="selected"</c:if>>已启用角色
    </option>
    <option value="2"
            <c:if test="${sysRoleEntity.state == 2}">selected="selected"</c:if>>已禁用角色
    </option>
</select>
</form>
<table border="1px" class="table">
    ...
    <c:forEach items="${pageModel.list }" var="role" varStatus="v">
        <tr>
            ...
            </a>　|　<a href="<%=request.getContextPath() %>/RoleServlet?command=updateMessage&id=${role.id}"
                      onclick="return confirm('是否修改角色？');">角色修改</a></td>
