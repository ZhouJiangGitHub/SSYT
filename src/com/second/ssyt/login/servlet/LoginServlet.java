...
public class LoginServlet extends HttpServlet {
     ...
     * @param req
     */
    public void udpate(HttpServletResponse res, HttpServletRequest req) {
        String phoneNumber = req.getParameter("phoneNumber");
        String userEmail = req.getParameter("userEmail");
        int userId = Integer.parseInt(req.getParameter("userId"));
        int result = loginDao.userUpdate(userId, phoneNumber, userEmail);

        if (result == 1) {
            UserEntity u = (UserEntity) req.getSession().getAttribute("user");
            u.setEmail(userEmail);
            u.setPhone(phoneNumber);
            req.getSession().setAttribute("user", u);
            try {
                req.getRequestDispatcher("/WEB-INF/view/common/main.jsp?updateState=" + result).forward(req, res);
